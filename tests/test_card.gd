...
const SINGLE_CARD_GROUP_NAME = "test_cards_single"
const PILE_CARD_GROUP_NAME = "test_cards_pile"
...
func _ready() -> void:
	single_card_stack = ClassCardStack.new()
	card_pile_stack = ClassCardStack.new()

	GameData.set_test_cards(SINGLE_CARD_GROUP_NAME, single_card_stack)
	GameData.set_test_cards(PILE_CARD_GROUP_NAME, card_pile_stack)
	...
	CardNodeManager.create_card_node(card_hearts_a, SINGLE_CARD_GROUP_NAME, "TestCard/Cards")
	...
	for point in points_copy:
		...
		CardNodeManager.create_card_node(new_card, PILE_CARD_GROUP_NAME, "TestCard/Cards")
...
func _item_changed_single_card():
	var card_nodes = CardNodeManager.get_card_nodes_by_group(SINGLE_CARD_GROUP_NAME)
...
func _item_changed_piled_cards():
	var card_nodes = CardNodeManager.get_card_nodes_by_group(PILE_CARD_GROUP_NAME)
	var index = 0
	for node in card_nodes:
