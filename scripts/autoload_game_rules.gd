...
# 检查是否能移动到目标牌
func check_card_move_tableau_legal(card_source:ClassCard,card_target:ClassCard)->bool:
	...
	if point_value_s==-1 or point_value_t==-1:
		return false
	return ((point_value_s==point_value_t-1) 
...
func deal()->Array[ClassCardStack]:
	...
	while deck_count>0:
		...
		# 随机加入到一个dealt_cards
		if table_to_deal_count[7]>0 and randi() % 2==0:
			# 50%的概率加入stock
